...
export class ConceptoEstratificacion {
  main: any;
  code: string;
  ...
  download(): any {
    ...
    doc.text('CERTIFICADO DE ESTRATIFICACIÓN', doc.internal.pageSize.width / 2, this.row, {
      align: 'center',
    });
    this.row += 3;
    ...
    const texto4 = `No. Predial Nacional: ${this.nomenclaturaEstratificacion.codigo_predial_nacional}`;
    doc.text(texto4, doc.internal.pageSize.width / 2, this.row, {
      align: 'center',
    });
    this.row += 10;

    doc.setFont(undefined, 'bold');
    const estrato = this.getDescEstrato();
    lines = `EL ESTRATO: ${estrato}`;
    ...
    doc.setFontSize(6);
    this.row += 10;
    doc.text('Con la presente certificación proceden los recursos de reposición ante la Alcaldía Municipal y el de Apelación ante el Comité permanente de estratificación, que se presentarán dentro de los diez (10) días siguientes a la fecha del presente documento.', lMargin, this.row, { maxWidth: 185, align: 'justify' });
    this.row += 10;
  ...
  getDescEstrato() {
    let desc;
    switch (this.nomenclaturaEstratificacion.estrato) {
      case '1':
        desc = 'UNO (1)';
        break;
      case '2':
